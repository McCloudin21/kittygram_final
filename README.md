...
Отличия docker-compose.yml от docker-compose.production.yml:

При запуске docker-compose.yml 
- Docker собирает контейнеры из образов на локальном компьютере

При запуске docker-compose.production.yml
- Нужно явно указывать этот файл через флаг `-f`
- Docker собирает контейнеры из образов с DockerHub
___
## Установка 
...
2. Выполните миграции, соберите статические файлы бэкенда и скопируйте их в /backend_static/static/:
